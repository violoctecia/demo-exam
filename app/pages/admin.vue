...
const filtered = computed(() => {
	...
	return orders.value.filter((o) => {
		...
		const matchSearch = !q || [o.login, o.first_name, o.last_name, o.email, o.course_name].some((f) => f?.toLowerCase().includes(q));
...
<template>
	<main class="container min-h-screen py-12!">
		...
		<h1 class="font-bold text-2xl mb-8">Панель администратора</h1>
		...
		<div class="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
			<div
				...
				:key="i"
				class="backdrop-blur-xl border border-border rounded-(--radius) p-4"
			>
		...
		<div
			v-if="pending"
			class="text-center py-20 text-text/40 animate-pulse"
		>
			Загрузка...
		</div>
		<div
			v-else-if="!filtered.length"
			class="text-center py-20 text-text/40"
		>
			Заявок не найдено.
		</div>
		...
		<div
			v-else
			class="backdrop-blur-xl bg-white/10 border border-border rounded-(--radius) shadow-xl overflow-hidden"
		>
					...
					<tbody class="divide-y divide-border">
						...
						>
							...
							<td class="px-5 py-4 align-top max-w-[200px]">
								...
								<p
									v-if="order.feedback"
									class="text-xs text-text/60 italic mt-2 border-l-2 border-primary/40 pl-2"
								>
									"{{ order.feedback }}"
								</p>
							</td>
							...
							<td class="px-5 py-4 align-top">
								<div class="flex items-center gap-2">
									...
									<svg
										v-if="updatingId === order.id_order"
		...
		>
			<span class="text-sm text-text/50">
				Страница
				<strong class="text-text">{{ currentPage }}</strong>
				из
